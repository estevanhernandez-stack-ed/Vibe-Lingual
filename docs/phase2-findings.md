...
**Phase-2 status (at run 2):** the plugin has extracted everything it can CONFIDENTLY do (41 components / 302 keys, green). Full completion of the heavy frontier needs the BUG-3 per-site-routing evolve pass OR manual extraction. Translation (es/fr/de/pt/it/ja/zh/ko/hi for the 302 keys) is the orthogonal follow-up axis.

## Run 3 (hardened: per-site routing + ignore-respect) — the frontier landed

BUG-3 fixed (per-SITE routing, commit `a047e23`) + built-in ignore-respect (`e81b3d0`); plugin at 315 tests. Re-ran Phase 2 on Celestia3: the heavy components now **partially localize** (clean sites extract, ambiguous left inline). **en catalog 302 → 720 keys** (+418), 26 files, 21 fully-extracted files guarded, suite green (592/592, lint 0, tsc clean). Shipped as Celestia3 **PR #94**. The heavy frontier (CosmicCalibration, AdminView, NumerologyView, OnboardingExperience, Oneirocriton, NatalCompass, DashboardShell, …) is now substantially localized.

**BUG-4 (recast entity-escaping, MEDIUM — open plugin-side):** partial extraction de-escaped some JSX HTML entities (`&amp;` → `&`, etc.) via the recast pass, producing 6 lint errors. Worked around ON THE CELESTIA3 BRANCH (`db87551` re-escaped them) — but the **plugin codemod still de-escapes entities**; it's not fixed in `transform.mjs`. **Fix direction:** the recast/print pass must preserve original JSX entity escaping (don't normalize `&amp;`/`&lt;`/`&gt;`/`&#...;` in untouched text); only the extracted literals change. Add a fixture: a JSX text node containing `&amp;` adjacent to an extracted sibling → entity preserved. A release-hardening target.

**One transient note:** the run-3 extract agent hit a 529 (API overload) mid-run; the work had already committed, the later phases + controller verification confirmed the green end state. No corruption — the branch + PR #94 are coherent.

**Phase-2 status (at run 3):** ~67 components touched, **720 en keys** localized (the bulk of the app's UI strings). Remaining: the inline-only low-confidence sites (locale-sensitive dates needing `useFormatter` + the timeZone decision, long interpolated strings) — a later targeted pass; and translation (9 locales for the 720 keys). Open plugin-side targets for release: BUG-4 (entity escaping) + a `useFormatter` date-extraction path for the inline date sites.
