...
```
The `WiredFileSet` carries: `request.ts` (AVAILABLE-list guard + try/catch source-locale fallback — a selected-but-missing catalog never crashes the request), `locale-cookie.ts` (the single cookie-name source), the layout `<NextIntlClientProvider>` mount patch, the `next.config` `createNextIntlPlugin` patch, and **the jest config patch** — which does two things: injects the `transformIgnorePatterns` ESM allowlist (next-intl 4.x is ESM-only — the existing suite breaks on first run without it) AND adds `<rootDir>/.vibe-lingual/` to `testPathIgnorePatterns` so jest never globs the plugin's own per-file backups (which mirror `*.test.tsx` copies and would fail as bare, pre-wrap suites — the Celestia3 dogfood found this). Reuse a detected language list (e.g. `SUPPORTED_LANGUAGES`) for the UI locale set; model UI locale SEPARATELY from any content/output locale. Apply the new files + patches with the same backup discipline as extract.
...
**The wrapper seeds real messages + a non-throwing fallback (Celestia3 dogfood fix).** next-intl 4.x does NOT echo a missing key — an unresolved `t('foo')` THROWS `MISSING_MESSAGE` and turns the wrapped test red, the exact failure the wrap exists to prevent. So the engine seeds the provider's `messages` with the namespace block it just extracted (the component's own `t()` keys resolve to the real source strings, keeping existing `getByText` assertions intact) AND adds `onError={() => {}}` + `getMessageFallback={({ key }) => key}` so any key outside the seeded set degrades to the key string instead of throwing. You only hand-fill messages for a sibling namespace a render also needs.
