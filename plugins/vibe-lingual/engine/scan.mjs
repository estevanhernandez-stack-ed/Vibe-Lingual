...
import { scanXaml } from './scan-xaml.mjs';
...
// The user-facing-text gates (hasLetter / CSS-token / identifier shapes / the
// short-fragment floor), key suggestion, and confidence scoring moved to
// text-heuristics.mjs (2026-09-05, WPF stack change) so the JSX and XAML scanners
...
export function scan(root, detection) {
  // Stack dispatch (2026-09-05): a WPF app's surface is XAML, which the Babel
  // pass below cannot see (the RoRoRo dogfood scanned 395 literals as 0 sites).
  // The XAML scanner produces the same inventory shape; everything downstream of
  // this branch is the unchanged JS/TS path.
  if (detection && detection.app && detection.app.stack === 'wpf') {
    return scanXaml(root, detection);
  }
